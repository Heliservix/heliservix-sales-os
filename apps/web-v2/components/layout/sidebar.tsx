  ...
  return (
    <aside className="hidden h-screen w-[21rem] shrink-0 border-r border-brand-blue/20 bg-brand-navy px-4 py-5 text-white shadow-2xl shadow-brand-navy/18 lg:sticky lg:top-0 lg:flex lg:flex-col">
          ...
          <section key={group.label}>
            ...
            <div className="space-y-1">
              {group.items.map((item) => {
                ...
                const isActive = !isPlanned && (item.href === "/" ? pathname === "/" : pathname.startsWith(item.href));

                return (
                  <a
                    key={item.label}
                    href={isPlanned ? undefined : item.href}
                    ...
                    onClick={(event) => {
                      if (isPlanned) event.preventDefault();
                    }}
                  >
                    ...
                    {item.status === "ready" ? (
                      ...
                      <span className="rounded border border-white/10 px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-white/38">
                        {t("status.planned")}
                      </span>
                    )}
                  </a>
      ...
      <div className="mt-5 rounded-xl border border-white/10 bg-white/[0.05] p-4 shadow-control">
        <StatusPill tone="teal">HSV OS 0.3</StatusPill>
